...
with col1:
    ...
    ax.add_patch(plt.Rectangle((px - 0.02, py - 0.02), pw + 0.04, ph + 0.04, color="#333533", zorder=2))
    ax.add_patch(plt.Rectangle((px, py), pw, ph, color="#003566", zorder=3))

    for i in range(1, 10):
        ax.plot([px + i * pw / 10, px + i * pw / 10], [py, py + ph], color="#4CC9F0", linewidth=0.5, alpha=0.3, zorder=4)

    if light_intensity > 0:
        for tx in np.linspace(px + 0.05, px + pw - 0.05, 7):
            for lw, a in zip([8, 4, 1.5], [0.05, 0.1, 0.3]):
                ax.plot([sun_x, tx], [sun_y, cy], color="#FFD60A", alpha=a * (light_intensity / 1.5), linewidth=lw * light_intensity, zorder=1)
    ...
    ax.add_patch(Arc((cx, cy), 0.25, 0.25, angle=0, theta1=0, theta2=angle_deg, color="#FFD60A", linewidth=2, alpha=0.8))

    t_a = np.radians(angle_deg / 2)
    ax.text(cx + 0.35 * np.cos(t_a), cy + 0.35 * np.sin(t_a), f"{angle_deg}°", color="white", fontsize=10, ha="center", fontweight="bold")
...
with col2:
    ...
    with r1c1:
        ...
        st.latex(r"V_{oc} = V_{oc,0} \left(1 + 0.2 \ln(I_L + 1)\right)")
        st.info(f"{Voc:.3f} V")
    with r1c2:
